...
class PointTokenizeEncoder(nn.Module):
    ...
    def forward(self, obj_pcds, obj_locs, obj_masks, obj_sem_masks,
        ...
        # print(obj_pcds.size()) #[batch,object,point(1024),feature(6)]
        ...
        # print(f'raw:{obj_embeds.shape}') #[batch,object,hidden_dim(768)]
        if self.freeze:
            obj_embeds = obj_embeds.detach()

        # get semantic cls embeds
        obj_sem_cls = F.softmax(self.point_cls_head(obj_embeds), dim=2).detach()  # B, N, O
        # print(obj_sem_cls.shape) [B,O,607]
        ...
        obj_embeds = obj_embeds + obj_sem_cls_embeds #B,O,768

        # get semantic mask embeds
        # print(obj_sem_masks[1][-4],obj_sem_masks.size())
        # Each Scan has some invalid objects due to padding, obj_sem_masks which has the length of the number of the true object is designed to filter out the invalid object
        ...
        # print(obj_sem_mask_embeds[1][-2])
        obj_embeds = obj_embeds + obj_sem_mask_embeds #give invalid object a specific embedding
        ...
        for i, pc_layer in enumerate(self.spatial_encoder):
            ...
            # print(obj_embeds[0][0],pairwise_locs.shape,obj_masks.shape)
        ...
        return obj_embeds, obj_embeds_pre, obj_sem_cls #obj_embeds_pre is the embedding before spatial transformer, obj_sem_cls is the predicted class of the object