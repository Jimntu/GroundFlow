...
class TransformerSpatialDecoderLayer(TransformerDecoderLayer):
    ...
    def forward(
        ...
        # print(memory_key_padding_mask.sum(dim=-1)) # 0 for all padding samples
        tgt2, cross_attn_matrices = self.multihead_attn(
            query=tgt2, key=memory,
            value=memory, attn_mask=memory_mask,
            key_padding_mask=memory_key_padding_mask
...
class TransformerSpatialDecoderLayerV2(TransformerDecoderLayer):
    def __init__(
        self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu",
        spatial_multihead=True, spatial_dim=5, spatial_attn_fusion='mul'
    ):
    ...
    def forward(
        ...
        valid_sample_mask = memory_key_padding_mask.sum(dim=-1) < memory_key_padding_mask.size(-1)
        # print(memory_key_padding_mask.shape)
        # print(memory_key_padding_mask.sum(dim=-1))
        # print(valid_sample_mask)
        
        valid_tgt2 = tgt2[valid_sample_mask]
        valid_memory = memory[valid_sample_mask]
        valid_memory_mask = memory_mask[valid_sample_mask] if memory_mask is not None else None
        valid_memory_key_padding_mask = memory_key_padding_mask[valid_sample_mask]

        valid_tgt2, valid_cross_attn_matrices = self.multihead_attn(
            query=valid_tgt2, key=valid_memory,
            value=valid_memory, attn_mask=valid_memory_mask,
            key_padding_mask=valid_memory_key_padding_mask
        )
        # Place results back into the original tensor
        tgt2[valid_sample_mask] = valid_tgt2
        ...
        tgt2 = self.linear2(self.dropout(self.activation(self.linear1(tgt2))))
        tgt = tgt + self.dropout3(tgt2)
        return tgt, self_attn_matrices, valid_cross_attn_matrices

@GROUNDING_REGISTRY.register()
class CMT_encoder_single(nn.Module):

    def __init__(self, config,spatial_dec= True, hidden_size = 768, num_attention_heads = 12, dim_loc=6,spatial_multihead= True, spatial_dim =5, spatial_dist_norm =True, spatial_attn_fusion = 'cond', num_layers =4, obj_loc_encoding = 'same_all', pairwise_rel_type= 'center'):
        super().__init__()
        self.config = config
        self.spatial_dec = spatial_dec
        self.obj_loc_encoding = obj_loc_encoding
        self.pairwise_rel_type = pairwise_rel_type
        self.spatial_dist_norm = spatial_dist_norm
        self.spatial_dim = spatial_dim
        if spatial_dec:
        ...
        if obj_loc_encoding in ['same_0', 'same_all']:
            num_loc_layers = 1
        elif obj_loc_encoding == 'diff_all':
            num_loc_layers = num_layers
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            if self.obj_loc_encoding == 'diff_all':
                ...
                if self.obj_loc_encoding == 'same_all':
                    out_embeds = out_embeds + query_pos
                else:
                    if i == 0:
                        out_embeds = out_embeds + query_pos
...
@GROUNDING_REGISTRY.register()
class CMT_encoder_add(nn.Module):
    ...
    def __init__(self, config,spatial_dec= True, hidden_size = 768, num_attention_heads = 12, dim_loc=6,spatial_multihead= True, spatial_dim =5, spatial_dist_norm =True, spatial_attn_fusion = 'cond', num_layers =4, obj_loc_encoding = 'same_all', pairwise_rel_type= 'center'):
        ...
        self.spatial_dim = spatial_dim
        if spatial_dec:
            decoder_class = TransformerSpatialDecoderLayerV2
            kwargs = {
                'spatial_dim': spatial_dim,
                'spatial_multihead': spatial_multihead,
                'spatial_attn_fusion': spatial_attn_fusion,
    ...
    def forward(
        self,pre_joint_embeds, txt_embeds, txt_masks, obj_embeds, obj_locs, obj_masks
    ):
        if self.spatial_dec:
            pairwise_locs = self.calc_pairwise_locs(
                obj_locs[:, :, :3], obj_locs[:, :, 3:], 
        ...
        if pre_joint_embeds is None:
            pre_joint_embeds = torch.zeros(out_embeds.shape).cuda()
        out_embeds = out_embeds + pre_joint_embeds
        
        return out_embeds
    

@GROUNDING_REGISTRY.register()
class CMT_encoder_CT(nn.Module):
    ...
    def __init__(self, config,spatial_dec= True, hidden_size = 768, num_attention_heads = 12, dim_loc=6,spatial_multihead= True, spatial_dim =5, spatial_dist_norm =True, spatial_attn_fusion = 'cond', num_layers =4, obj_loc_encoding = 'same_all', pairwise_rel_type= 'center'):
        ...
        history_fusion_layer = CrossAttentionLayer(hidden_size, num_attention_heads, dim_feedforward=2048)
    ...
    def forward(
        self,pre_joint_embeds, pre_joint_masks, txt_embeds, txt_masks, obj_embeds, obj_locs, obj_masks
    ):
        if self.spatial_dec:
            pairwise_locs = self.calc_pairwise_locs(
                obj_locs[:, :, :3], obj_locs[:, :, 3:], 
        ...
        if pre_joint_embeds is not None:
            for i, history_encoder in enumerate(self.history_encoder):
                out_embeds,_ = history_encoder(out_embeds,pre_joint_embeds)
                # out_embeds,_ = history_encoder(out_embeds,pre_joint_embeds,memory_key_padding_mask = pre_joint_masks.logical_not())
        
        return out_embeds,obj_masks
    
@GROUNDING_REGISTRY.register()
class CMT_encoder_GF(nn.Module):
    ...
    def __init__(self, config,spatial_dec= True, hidden_size = 768, num_attention_heads = 12, dim_loc=6,spatial_multihead= True, spatial_dim =5, spatial_dist_norm =True, spatial_attn_fusion = 'cond', num_layers =4, obj_loc_encoding = 'same_all', pairwise_rel_type= 'center'):
        ...
        # history_fusion_layer = CrossAttentionLayer(hidden_size, num_attention_heads, dim_feedforward=2048)
        history_fusion_layer = TransformerDecoderLayer(hidden_size, num_attention_heads,
            dim_feedforward=2048, dropout=0.1, activation='gelu')
        self.history_encoder = layer_repeat(history_fusion_layer, 1)
    ...
    def forward(
        self, history_joint_features, pre_joint_features, pre_joint_masks, txt_embeds, txt_masks, obj_embeds, obj_locs, obj_masks
    ):
        if self.spatial_dec:
            pairwise_locs = self.calc_pairwise_locs(
                obj_locs[:, :, :3], obj_locs[:, :, 3:], 
        ...
        if pre_joint_features is not None:
            if history_joint_features is not None:
                joint_embeds_normalized = F.normalize(out_embeds, p=2, dim=-1)
                history_joint_features_normalized = F.normalize(history_joint_features, p=2, dim=-1)
                # print(joint_embeds_normalized.shape,history_joint_features_normalized.shape)
                cosine_similarity = torch.bmm(joint_embeds_normalized, history_joint_features_normalized.transpose(1, 2))
                a = cosine_similarity.mean(dim=1, keepdim=True).transpose(1,2) #v27->max v25->mean
                # print(a.shape) #[B,T+O,1]
                pre_joint_features = pre_joint_features + a*history_joint_features
                # pre_joint_embeds = F.normalize(pre_joint_embeds, p=2, dim=-1) #v22

            for i, history_encoder in enumerate(self.history_encoder):
                out_embeds, self_attn_matrices, cross_attn_matrices = history_encoder(
                    out_embeds, pre_joint_features,
                    tgt_key_padding_mask=obj_masks.logical_not(),
                    memory_key_padding_mask=pre_joint_masks.logical_not(),
                )
