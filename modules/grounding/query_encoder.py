...
@GROUNDING_REGISTRY.register()
class QueryMaskEncoder_GroundFlow(nn.Module): #v25
    def __init__(self, cfg, memories=[], memory_dropout=0.0, hidden_size=768, num_attention_heads=12, num_layers=4,
        ...
        history_encoder = CrossAttentionLayer(hidden_size, num_attention_heads, dim_feedforward = 2048)
        self.history_encoder = layer_repeat(history_encoder, num_layers)
    ...
    def forward(self, history_query, pre_query, pre_query_masks, input_dict, pairwise_locs, mask_head=None):
            
        predictions_class, predictions_mask = [], []
        
        query, query_masks, _ = input_dict['query']
        voxel_feat = input_dict['voxel'][0] if 'voxel' in input_dict.keys() else None
        ...
        for i, layer in enumerate(self.unified_encoder):
            ...
            if self.use_self_mask:
                attn_mask[attn_mask.all(-1)] = False 
            ...
            if isinstance(voxel_feat, list):
                input_dict['voxel'][0] = voxel_feat[i] 
        ...
        if pre_query is not None:
            if history_query is not None:
                joint_embeds_normalized = F.normalize(query, p=2, dim=-1)
                history_joint_features_normalized = F.normalize(history_query, p=2, dim=-1)
                ...
                pre_query = pre_query + a*history_query
            for i, history_encoder in enumerate(self.history_encoder):
                    query,_ = history_encoder(query,pre_query,memory_key_padding_mask = pre_query_masks)
    
        return query, query_masks, predictions_class, predictions_mask
    
@GROUNDING_REGISTRY.register()
class QueryMaskEncoder_Single(nn.Module):
    def __init__(self, cfg, memories=[], memory_dropout=0.0, hidden_size=768, num_attention_heads=12, num_layers=4,
                share_layer=False, spatial_selfattn=False, structure='sequential', drop_memories_test=[], use_self_mask=False, num_blocks=1):
        super().__init__()

        self.spatial_selfattn = spatial_selfattn #True
        query_encoder_layer = QueryEncoderLayer(hidden_size, num_attention_heads, memories, spatial_selfattn=spatial_selfattn, structure=structure, memory_dropout=memory_dropout, drop_memories_test=drop_memories_test)
        self.unified_encoder = layer_repeat(query_encoder_layer, num_layers, share_layer)
...
class QueryEncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, memories, dim_feedforward=2048, dropout=0.1, activation="relu", prenorm=False, spatial_selfattn=False, structure='mixed', memory_dropout=0, drop_memories_test=[]):
        ...
        cross_attn_layer = CrossAttentionLayer_qfusion(d_model, nhead, dropout=dropout, activation=activation, normalize_before=prenorm, batch_first=True) 
        # cross_attn_layer = CrossAttentionLayer( d_model,  nhead, dim_feedforward)
    ...
    def forward(self, query, input_dict, pairwise_locs=None):
        ...
        def sequential_ca(query, memories):
            for memory in memories:
                ...
                # for tokens in memory_key_padding_mask:
                #     if not any(tokens):
                #         tokens[0] = True
                # query,_ = cross_attn(tgt=self.with_pos_embed(query,query_pos), memory=self.with_pos_embed(feat,pos), memory_mask=attn_mask, memory_key_padding_mask = memory_key_padding_mask)
            return query
        ...
        def parallel_ca(query, memories):
            ...
            for memory in memories:
                ...
                #     if not any(tokens):
                #         tokens[0] = True
                # update,_ = cross_attn(tgt=self.with_pos_embed(query,query_pos), memory=self.with_pos_embed(feat,pos), memory_mask=attn_mask, memory_key_padding_mask = memory_key_padding_mask)
        ...
        if self.structure == 'sequential':
            ...
            # [mv,pc,vx] + prompt
            query = parallel_ca(query, [m for m in memories if m != 'prompt']) #pc
...
class CrossAttentionLayer_qfusion(nn.Module):
    def __init__(
        self,
        d_model,
        nhead,
    ...
    def forward(
        ...
        # print(self.normalize_before) False
        if self.normalize_before:
            return self.forward_pre(
                tgt,
                memory,
