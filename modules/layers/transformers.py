...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        # Performance boosts after normalization and mlp
        if not self.prenorm:
            tgt = self.norm1(tgt)
        if self.prenorm:
            tgt2 = self.norm3(tgt)
...
class Gated_CrossAttentionLayer(nn.Module):
...
class DualstreamLayer(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048): 
        super().__init__()
        self.ct1 = CrossAttentionLayer(d_model, nhead, dim_feedforward=dim_feedforward)
        self.ct2 = CrossAttentionLayer(d_model, nhead, dim_feedforward=dim_feedforward)
        self.st1 = TransformerEncoderLayer(d_model, nhead, dim_feedforward=dim_feedforward)
        self.st2 = TransformerEncoderLayer(d_model, nhead, dim_feedforward=dim_feedforward)
        
    def forward(
            self, t1, t2,
            t1_key_padding_mask: Optional[Tensor] = None,
            t2_key_padding_mask: Optional[Tensor] = None
    ):
        # nan_locations = torch.isnan(t1).nonzero(as_tuple=False)
        # print(f't1:{nan_locations}')
        # nan_locations = torch.isnan(t2).nonzero(as_tuple=False)
        # print(f't2:{nan_locations}')
        # print(f't1:{torch.isnan(t1).sum()}, t2:{torch.isnan(t2).sum()}')  # Check number of NaNs
        tgt1, attn_ct1 = self.ct1(t1,t2,memory_key_padding_mask=t2_key_padding_mask)
        tgt2, attn_ct2 = self.ct2(t2,t1,memory_key_padding_mask=t1_key_padding_mask)
        
        tgt1, attn_st1 = self.st1(tgt1,tgt_key_padding_mask=t1_key_padding_mask)
        tgt2, attn_st2 = self.st2(tgt2,tgt_key_padding_mask=t2_key_padding_mask)
        tgt1 = torch.where(torch.isnan(tgt1), torch.tensor(-100.0), tgt1)
        tgt2 = torch.where(torch.isnan(tgt2), torch.tensor(-100.0), tgt2)
        # print(tgt2.shape,attn_st2.shape) [B,T2,H], [B,T2,T2]
        # print(f'tgt1:{torch.isnan(tgt1).sum()}, tgt2:{torch.isnan(tgt2).sum()}')
        return tgt1,tgt2     
...
class TransformerEncoderLayer(nn.Module): #Self-attention
    ...
    def forward(
        ...
        tgt = tgt + self.dropout2(tgt2)
        # nan_locations = torch.isnan(tgt).nonzero(as_tuple=False)
        # print(nan_locations)
        if not self.prenorm:
            tgt = self.norm2(tgt)
        # print(self_attn_matrices,self_attn_matrices.shape)
...
if __name__ == '__main__':
    x = DualstreamLayer(768,12).cuda()
    t1 = torch.rand((32, 100, 768)).cuda()
    t1_mask = torch.ones((32, 100)).cuda()
    t2 = torch.rand((32, 100, 768)).cuda()
    t2_mask = torch.ones((32, 100)).cuda()
    tgt1,tgt2 = x(t1, t2, t1_mask, t2_mask)
    print(torch.isnan(tgt1).nonzero(as_tuple=False))
    print(torch.isnan(tgt2).nonzero(as_tuple=False))