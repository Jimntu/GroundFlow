...
def layer_repeat(module, N, share_layer=False):
    if share_layer:
        return nn.ModuleList([module] * N) #share_weights
    else:
        return nn.ModuleList([copy.deepcopy(module) for _ in range(N - 1)] + [module]) #independent weights
...
def calc_pairwise_locs(obj_centers, obj_whls, eps=1e-10, pairwise_rel_type='center', spatial_dist_norm=True,
    ...
    # print(pairwise_locs.shape) (B,#O,#O,5)
    return pairwise_locs
