...
class GroundHeadV1(nn.Module):
    ...
    def forward(self, txt_embeds, obj_embeds, obj_pre_embeds, obj_masks, **kwargs):
        ...
        # print(f'og3d_logits:{og3d_logits},obj_masks:{obj_masks}')
        og3d_logits = og3d_logits.masked_fill_(obj_masks.logical_not(), -float('inf'))
        # print(f'og3d_logits_after_mask:{og3d_logits}')
        ...
        obj_cls_logits = self.obj3d_clf_head(obj_embeds) #[607,1]
        obj_cls_pre_logits = self.obj3d_clf_pre_head(obj_pre_embeds) #[607,1] without spatial transformer
        # print(txt_cls_logits.shape,obj_cls_logits.shape,obj_cls_pre_logits.shape,og3d_logits.shape)
        # [B,O],[B,N,O],[B,N,O],[B,N]
        return txt_cls_logits, obj_cls_logits, obj_cls_pre_logits, og3d_logits

@HEADS_REGISTRY.register()
class GroundHead_mikasa(nn.Module):
    def __init__(self, cfg, input_size=768, hidden_size=768, sem_cls_size=607, dropout=0.3, detach_all_aux_loss=False):
        super().__init__()
        self.og3d_head = get_mlp_head(
            input_size, hidden_size,
    ...
    def forward(self, txt_embeds, obj_embeds, obj_pre_embeds, obj_masks, **kwargs):
        ...
        obj_embeds = obj_embeds + obj_pre_embeds
