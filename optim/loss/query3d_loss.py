...
def cross_entropy(logits, label):
    ...
    if label.shape == logits.shape: # label is a 0-1 vector and we use BCE loss. L=−(y⋅log(σ(x))+(1−y)⋅log(1−σ(x)))
        ...
        logits = logits.view(-1, logits.shape[-1])
        label = label.view(-1)
        return F.cross_entropy(logits, label,ignore_index=-100)  #vista use this, ignore the padding label when using
