...
class ScanFamilyDatasetWrapper(Dataset):
    def __init__(self, cfg, dataset):
        ...
        self.encoded_input_list = []
        # self.encoded_input_mask = []
        self.tokenized_step = self.tokenize_step()
        
    

    def tokenize_step(self):
        for i in range(len(self.dataset)):
            step_list = self.dataset.get_lang(i)[-1]['step_instruction']
            encoded_input = self.tokenizer([step_list[j] for j in range(len(step_list))], 
                                        add_special_tokens=True, truncation=True,return_tensors='pt',padding=True)
            self.encoded_input_list.append(encoded_input.input_ids.float())
        return self.encoded_input_list
    ...
    def __getitem__(self, idx):
        ...
        data_dict['step_prompts']= self.tokenized_step[idx]
...
@DATASETWRAPPER_REGISTRY.register()
class ScanFamilyDatasetWrapper_Single(Dataset):
    def __init__(self, cfg, dataset):
        self.dataset = dataset
        tokenizer_name = getattr(cfg.data_wrapper, 'tokenizer', 'bert-base-uncased')
        self.tokenizer = AutoTokenizer.from_pretrained(tokenizer_name)
