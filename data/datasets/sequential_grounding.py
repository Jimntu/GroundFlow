...
# class SequentialGrounding(SceneVerseBase_aug):
    def __init__(self, cfg, dataset_name, split):
        ...
        self.lang_data, self.scan_ids = self._load_lang() #lang_data includes task descrip, action_steps, scan_id and item_id
        self.init_scan_data()

    def get_lang(self, index):  # used in sceneverseBase
        item = self.lang_data[index]
        task_description = item['task_description']
        action_steps = item['action_steps'] #list of dictionary, dic.keys() == action, label, target_id
        ...
        data_dict['step_instruction'] = [action['action'] + ' <s>' for action in action_steps]
        # data_dict['step_instruction'] = [task_description + ' <s> ' + action['action'] + ' <s>' for action in action_steps] 
        data_dict['step_num'] = len(data_dict['step_instruction'])
        # print(data_dict['step_instruction'])
        data_dict['source'] = self.dataset_name
        data_dict['data_idx'] = item['item_id']
        data_dict['label'] = tgt_object_name
    ...
    def _load_lang(self):
        ...
        if self.split == 'train':
            ...
            # anno_file_base = os.path.join(self.sequential_grounding_base, 'release_data', 'test_skip_step_implicit.json')
        ...
        for i, data in enumerate(anno_data):
            ...
            scan_ids.add(scan_id) # Each scan_ids have 5 tasks
            ...
            item['scan_id'] = scan_id # scan_id is unique for each scene
            item['item_id'] = f'f{scan_id}_{i}' # item_id is unique for each task
            if self.multi_step_context:
                lang_data.append(item) # include all action_steps, different from single step, which include only previous step information
            else:
                for j in range(len(item['action_steps'])):
                    new_item = deepcopy(item)
                    new_item['action_steps'] = [item['action_steps'][j]]
                    lang_data.append(new_item)   # Append action steps seperately     
...
class SequentialGroundingSingleStep(SceneVerseBase):
    ...
    def get_lang(self, index): #used in scenversebase
        item = self.lang_data[index]
    ...
    def _load_lang(self):
        ...
        for i, data in enumerate(anno_data):
            ...
            for j in range(len(item['action_steps'])):
                new_item = {}
                new_item['item_id'] = f'{scan_id}_{i}'
                ...
                if self.multi_step_context:
                    for k in range(j + 1):
                        utterance += ' ' + item['action_steps'][k]['action'] #Previous steps' action instruction, different for w/o singlestep
                else:
                    utterance += ' ' + item['action_steps'][j]['action']
                new_item['utterance'] = utterance
                # new_item['action_steps'] = data['action_steps']
