...
class SceneVerseBase(Dataset):
    ...
    def _load_split(self, cfg, split):
        if self.dataset_name == '3RScan':
            split_file = os.path.join(self.base_dir, f'3RScan/annotations/splits/{split}_split_non_overlap.txt')
    ...
    def _load_scans(self, scan_ids):
        ...
        if process_num > 1:
            ...
            for scan_id in unloaded_scan_ids:
                _, one_scan = self._load_one_scan(scan_id)
                scans[scan_id] = one_scan 
        print(f'Finish loading all the scans from {self.dataset_name}')
    ...
    def get_scene(self, scan_id, tgt_object_id_list, tgt_object_name_list, sentence):
        if not isinstance(tgt_object_id_list, list): #For Single Step setting, the id list and name list only contain 1 object
        ...
        data_dict = {
            ...
            "tgt_obj_boxes": tgt_obj_boxes.tolist(), 
    ...
    def match_gt_to_pred(self, scan_data):
        ...
        scan_data['matched_list'] = matched_list #list of the index of the pred box which matches the GT most
        scan_data['iou25_list'] = iou25_list #list of the list iou25 of pred box for each GT box
        scan_data['iou50_list'] = iou50_list #list of the list iou50 of pred box for each GT box
