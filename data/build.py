...
def get_dataset(cfg, split):
    ...
    for dataset_name, dataset in zip(cfg.data.get(split), dataset_list):
        ...
        data_proportion[dataset_name] = len(dataset) / total
    print(f'Total\t\t\t{total}')
    print('='*50)
    if split == 'train':
        dataset_list = ConcatDataset(dataset_list) #Treat multiple datasets as one.

    return dataset_list,data_proportion
...
def build_dataloader(cfg, split='train'):
    ...
    """
    if split == 'train':
        dataset,data_proportion = get_dataset(cfg, split)
        return DataLoader(dataset,
                          ...
                          shuffle=True,
                          drop_last=True),data_proportion
    else:
        loader_list = []
        dataset_list,data_proportion = get_dataset(cfg, split)
        ...
        if len(loader_list) == 1:
            return loader_list[0],data_proportion
        else:
            return loader_list,data_proportion
