...
class BaseTrainer():
    def __init__(self, cfg):
        ...
        for key in keys:
            data_info = build_dataloader(cfg, split=key)
            self.data_loaders[key] = data_info[0]
            self.data_proportion[key] = data_info[1]
        ...
        self.loss, self.optimizer, self.scheduler = build_optim(cfg, self.model.get_opt_params(), #return by the group of decay/no-decay para
        ...
        if cfg.get('pretrain_ckpt_path'):
            if 'sequential_grounding_base' in cfg.pretrain_ckpt_path:
                self.pretrain_ckpt_path = Path(cfg.pretrain_ckpt_path)
                self.load_pretrain()
            elif 'Results' in cfg.pretrain_ckpt_path:
                self.model.load_model(cfg.pretrain_ckpt_path)
